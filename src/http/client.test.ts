...
import { ForbiddenError, NetworkError, NotFoundError, RateLimitError, ValidationError } from '../errors';
import { apiRequest } from './client';

// Mock credentials so ForbiddenError can read the role
vi.mock('../auth/credentials', () => ({
  readCredentials: vi.fn(() => ({ role: 'analyst', accessToken: 'tok', refreshToken: 'ref', expiresAt: 9999999999999, username: 'user' })),
}));

// Mock token-refresher to avoid circular dependency issues in tests
vi.mock('./token-refresher', () => ({
  refreshAndRetry: vi.fn(async () => ({ status: 200, data: { retried: true }, headers: new Headers() })),
}));
...
describe('apiRequest', () => {
  ...
  it('throws ForbiddenError on 403', async () => {
    ...
    await expect(apiRequest(BASE_OPTIONS)).rejects.toBeInstanceOf(ForbiddenError);
  });
  ...
  it('NotFoundError id is last path segment', async () => {
    vi.mocked(fetch).mockResolvedValue(makeResponse(404));
    const err = await apiRequest({ ...BASE_OPTIONS, path: '/profiles/abc-123' }).catch(e => e);
    expect(err).toBeInstanceOf(NotFoundError);
    expect((err as NotFoundError).id).toBe('abc-123');
  });

  it('throws ValidationError on 422', async () => {
    vi.mocked(fetch).mockResolvedValue(makeResponse(422, { message: 'bad input', details: { name: ['too short'] } }));
    await expect(apiRequest(BASE_OPTIONS)).rejects.toBeInstanceOf(ValidationError);
  });

  it('ValidationError includes message and details', async () => {
    vi.mocked(fetch).mockResolvedValue(makeResponse(422, { message: 'bad input', details: { name: ['too short'] } }));
    const err = await apiRequest(BASE_OPTIONS).catch(e => e) as ValidationError;
  ...
  it('throws RateLimitError on 429 without Retry-After', async () => {
    ...
    const err = await apiRequest(BASE_OPTIONS).catch(e => e) as RateLimitError;
    expect(err).toBeInstanceOf(RateLimitError);
    expect(err.retryAfter).toBeUndefined();
  });

  it('RateLimitError includes retryAfter when header present', async () => {
    vi.mocked(fetch).mockResolvedValue(makeResponse(429, {}, { 'Retry-After': '30' }));
    const err = await apiRequest(BASE_OPTIONS).catch(e => e) as RateLimitError;
    expect(err).toBeInstanceOf(RateLimitError);
  ...
  it('NetworkError message includes operation name', async () => {
    vi.mocked(fetch).mockRejectedValue(new Error('ECONNREFUSED'));
    const err = await apiRequest({ ...BASE_OPTIONS, operation: 'list-profiles' }).catch(e => e) as NetworkError;
  ...
  it('sends JSON body with Content-Type header on POST', async () => {
    ...
    await apiRequest({ ...BASE_OPTIONS, method: 'POST', body: { name: 'Alice' } });
    const [, init] = vi.mocked(fetch).mock.calls[0]!;
